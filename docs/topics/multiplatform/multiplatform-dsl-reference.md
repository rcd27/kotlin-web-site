...
| `platformType`      | Designates the Kotlin platform of this target. Available values: `jvm`, `androidJvm`, `js`, `native`, `common`, `wasm`.                           |
...
Kotlin/Wasm targets that run in the JavaScript runtime. They can contain one of two blocks depending on the target execution
environment:
